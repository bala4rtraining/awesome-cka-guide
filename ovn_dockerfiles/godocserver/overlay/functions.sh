...
#date : 07/02/2019
#title : functions.sh
#description : collection of reusable functions for use within the OVN environment
#author		 : gabaguil
#usage		 : 1. load the script by invoking - source functions.sh
#              2. use functions normally
#===============================================================================================
...
#
get_slugs() {
    curl -s -o response-repos.json \
    -X GET \
    -u "$1" \
    -H "Content-Type: application/json" \
    $BASE_URL/rest/api/latest/projects/OP/repos?limit=200 
    jq -r .values[].slug response-repos.json > "$2"
    rm response-repos.json
}
...
#
get_categories() {
    curl -s -o response-categories.json \
    -X GET \
    -u "$1" \
    -H "Content-Type: application/json" \
    $BASE_URL/rest/categories/latest/project/OP/repository/"$2" 
    jq -r .result.categories[].title response-categories.json > "$3"
    rm response-categories.json
}
