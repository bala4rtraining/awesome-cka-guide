...
# Jenkinsfile:
#     This script is executed every 4 hours by https://jenkins-oce-corp.visa.com:8443/netproc/job/OVN/job/OVN_dev/job/Administration_Jobs/job/sonarqube_report/
# To run script:
...
if [[ $HTTPRC -eq 200 ]] ; then
    ...
    echo -e "${RED}ERROR${NC}: Unable to get project list. HTTPRC=${HTTPRC}"
    exit 1
fi
...
function getsonarqubestatus() {
    ...
    if [[ $HTTPRC -eq 200 ]] ; then
        ...
        echo -e "${YELLOW}WARNING${NC}: Unable to get issues list for ${projectkey}. HTTPRC=${HTTPRC}"
        myassigneeinfo="[]"
    fi
            ...
            if [[ $mybugsstring != "" ]]; then
                ...
                if [[ $myalertstatus != "" ]]; then
                    ...
                    if [[ $mycodesmells != "" ]]; then
                      ...
                      echo -e "${RED}WARNING${NC}: No code smell data available for project: $projectkey - please investigate "
                    fi
                else
                   echo -e "${RED}WARNING${NC}: No alert status available for project: $projectkey - please investigate "
                fi
            else
               echo -e "${RED}WARNING${NC}: No bugs count available for project: $projectkey - please investigate "
            fi
        else
           echo -e "${RED}WARNING${NC}: No coverage number available for project: $projectkey - please investigate "
        fi
    else
       echo -e "${YEL}WARNING${NC}: Unable to get coverage info for project: $projectkey. HTTPRC=${HTTPRC}"
       exit 1
    fi
}
