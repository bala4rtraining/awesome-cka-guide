...
-H 'Authorization: Bearer '$token'' \
-H 'Content-Type: application/csv' \
-H 'cache-control: no-cache' > /tmp/$scan_id

echo -e "\n\n-----Execute checkmarx_parser.py to create Jira Tickets add Author's field, Tickets in Checkmarx report-----"
/tmp/checkmarx_parser.py /tmp/$scan_id $project_name_checkmarx $jira_service_account_user $jira_service_account_password $checkmarx_project_prefix


inputfile="/tmp/$scan_id"
