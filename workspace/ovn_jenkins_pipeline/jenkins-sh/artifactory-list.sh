...
#
    ...
    grep 'ovn_clearing_xdc_sync-0' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep 'ovn_cas_tools-v0' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep 'ovn_clearing_jobs-0' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep 'ovn_nginx_auth-v0' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'bridgeeafetch-.*rpm' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'cfprocessor-.*rpm' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'ovn_switch-0.*.tar.gz' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'ovn_mediator-0.*.tar.gz' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'ovn_umf_delivery-0.*.tar.gz' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'ovn_multidc_sync-0.*.tar.gz' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'ovn_vitalsigns_delivery-0.*.tar.gz' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'ovn_vip_ef_sync-0.*.tar.gz' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'vss-2.*rpm' /tmp/ovn-app-el7.txt | sort -V | tail -$num
    grep -x 'vsswrapper-0.*rpm' /tmp/ovn-app-el7.txt | sort -V | tail -$num
