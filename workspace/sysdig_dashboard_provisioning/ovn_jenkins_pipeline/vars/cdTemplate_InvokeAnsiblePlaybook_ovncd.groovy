...
pipeline {
  ...
  parameters {
        string(name: 'extra_vars', defaultValue: '', description: 'Example: -e a=1 -vvv --become')
        ...
        stage("GET/SET playbook branch") {
                ...
                stage("Checkout playbook repository") {
                     ...
                     stage('Run ansible playbook') {
                         steps {
                             ansiColor('xterm') {
                               ansiblePlaybook colorized: true, credentialsId: 'ovncd_ssh_key', inventory: "inventories/${env.CLUSTER}/${env.DATACENTER}", playbook: "$playbookName", extras: "--become ${extra_vars} ${params.extra_vars}"
                             }
                         }
                     }
                 }
